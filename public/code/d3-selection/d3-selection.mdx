# Selection Method
* d3.select(selector), 从当前document root element 开始, 选择第一个满足 selector 的元素. 如果 selector 为空，返回 root element
* d3.selectAll(selector), 选择所有满足 selector 的元素
* selection.select(selector), 对 selection 里每一个元素选择其第一个满足 selector 的子元素，如果不存在，则返回 null, 如果存在，且当前元素有绑定数据，则该数据会被传递给子元素
* selection.selectAll(selector), 对 selection 里每一个元素选择其所有满足 selector 的子元素，返回的selection 是 grouped, 且元素数据不会被传递给选中的子元素

# Sample
<CodePen html="/code/d3-selection/d3-selection.html" deps="/code/d3-selection/d3-selection.json" code="/code/d3-selection/d3-selection.jsx"/>
...
datums 也可以是函数，且参数为当前选中元素的 parent selection node 的 datum.  
data 方法会返回一个新的 selection， 该 selection 为 update selection, 即由 selection中和 data 元素成功一一绑定的 element 组成  
当 datums 的size > selection.size(), 绑定的数据多于当前的selection node size, 可以通过 <code>enter()</code> 进入 enter selection. 即 focus 在额外的 datum 上   
...
当datums size < selection.size(), 可以通过 <code>exit()</code> 进入 exit selection, 即 focus 在额外的 selection node 上, 该 selection 通常后跟 <code>remove()</code> 来删除没有绑定数据的nodes  
...
d3.select('div')
    ...
    .join('p')//相当于call enter, 然后 merge update and enter selection
        .text((d, i) => d + '' + i)
```
上述的写法会自动将 enter 生成的 nodes 与 update selection nodes merge, 然后执行之后的操作，但是有的时候 enter 生成的 nodes 和 update selection nodes 会有不同的操作， 这个时候就需要更精细的写法。  
```js
d3.select('div')
    .selectAll('p')
    .data([1, 2, 3, 4, 5])
    .join(//set update/enter/exit explicitly 
        enter => enter.append('p').text((d, i) => d + '-' + i),
        update => update.text((d, i) => d + ' ' + i), 
        exit => exit.remove())
```
